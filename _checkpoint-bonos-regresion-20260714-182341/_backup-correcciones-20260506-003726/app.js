...
const permissionSections = ["agenda", "pacientes", "equipo", "automatizaciones", "facturacion", "rendimiento", "estadisticas"];
...
function availabilityBlocksFor(practitionerId, dateValue) {
  return availabilityBlocks.filter((block) => block.practitionerId === practitionerId && block.date === dateValue);
}
...
function availabilityBlockLabel(block) {
  ...
  return `${label} Â· ${hours}${block?.reason ? ` Â· ${block.reason}` : ""}`;
}
...
function renderWeekAppointment(appointment) {
  ...
  button.innerHTML = `
    <time><i></i>${appointment.start} - ${appointmentEnd(appointment)} Â· ${practitioner?.name || "Profesional"}</time>
...
function renderGroupBlock(group, dateValue = selectedDate, mode = "day") {
  ...
  card.innerHTML = `
    ...
    <span>${service?.name || "Servicio grupal"} Â· ${groupEnrollmentLabel(group)}</span>
...
function showGroupSummary(group, dateValue = selectedDate) {
  ...
  $("#group-session-meta").textContent = `${service?.name || "Servicio"} Â· ${weekDayLabels[dayKeyFor(dateValue)] || dateValue} ${dateValue} Â· ${group.start} - ${groupEnd(group)} Â· ${practitioner?.name || "Profesional"} Â· ${room?.name || "Sala"}`;
...
function renderGroupSessionPanel(group, dateValue) {
    ...
    ? fixedPatients.map((patient) => `
      <article class="compact-item dropin-item">
        <div>
          ...
          <span>Fijo mensual Â· ${patient.phone || "sin telefono"}</span>
        </div>
      </article>
    `).join("")
  ...
  extrasList.innerHTML = extras.length
    ? extras.map((entry) => {
        ...
        return `<article class="compact-item dropin-item"><div><strong>${patient?.name || entry.name || "Paciente"}</strong><span>Sesion suelta Â· ${entry.createdAt || ""}</span></div><button class="secondary-button" type="button" data-dropin-remove="${entry.id}">Quitar</button></article>`;
      }).join("")
  ...
  patients
    ...
    .forEach((patient) => dropinSelect.append(new Option(`${patient.name} Â· ${patient.phone || "sin telefono"}`, patient.id)));
  ...
  $("#services-table").innerHTML = services
    .map((service) => `
      <tr>
        ...
        <td>
          ${service.type === "group"
            ? `Sesion grupal Â· ${service.capacity || 1} plazas Â· ${service.dropInPrice || service.price || 0} EUR/sesion Â· ${service.monthlyPrice || 0} EUR/mes Â· ${service.commissionPerPatient || 0}% prof.`
            : `${service.price || 0} EUR`}
        </td>
...
function renderPerformance() {
  ...
  const activityItems = [
    ...
    ...(workerReport.groupSessions || []).map((session) => ({
      ...
      detail: `${session.serviceName} - ${session.roomName} Â· ${session.attendees} asistentes Â· ${session.revenue} EUR`
    }))
  ];
...
function renderReminderCard(reminder, mode = "pending") {
  ...
  article.innerHTML = `
    ...
    <small>Enviar: ${sendAt} Â· Cita: ${reminder.date}, ${reminder.start} Â· Tel: ${phone}</small>
  `;
  ...
  if (mode === "pending") {
    ...
    actions.innerHTML = `
      ...
      <button class="secondary-button" type="button" data-reminder-action="confirmed">Confirmado</button>
...
function setupDialog() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (!isWithinAvailability(candidate)) {
      $("#form-error").textContent = `${practitioner?.name || "El profesional"} trabaja en este horario: ${practitionerAvailabilityLabel(practitioner)}.`;
      $("#form-error").classList.add("visible");
      return;
    }
    ...
    if (conflict) {
      ...
      $("#form-error").textContent = `Conflicto detectado con la cita de ${patient} a las ${conflict.start}.`;
      $("#form-error").classList.add("visible");
      return;
    }
    ...
    if (groupConflict) {
      $("#form-error").textContent = `Ese hueco esta bloqueado por el grupo ${groupConflict.name} a las ${groupConflict.start}.`;
      $("#form-error").classList.add("visible");
      return;
    }
    ...
    if (apiEnabled && isDemoClinic() && candidate.serviceId && byId(services, candidate.serviceId)) {
      apiRequest("/api/appointments", {
        method: "POST",
...
function fillServiceForm(form, service) {
  ...
  form.elements.capacity.value = service.capacity || 1;
...
function setupServiceDialog() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    const localService = {
      ...
      type: serviceType,
      capacity: serviceType === "group" ? Math.max(1, Number(form.elements.capacity?.value || 1)) : 1,
...
function refreshGroupFormOptions(form = $("#group-form")) {
  ...
  patients.forEach((patient) => patientsSelect.append(new Option(`${patient.name} Â· ${patient.phone || "sin telefono"}`, patient.id)));
}
