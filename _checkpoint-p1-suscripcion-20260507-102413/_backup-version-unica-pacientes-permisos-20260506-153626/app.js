...
function renderPermissions() {
  ...
  list.innerHTML = [
    permissionRowHtml("staff", "Recepcion / empleado", permissionSettings.staff),
    ...practitioners.map((practitioner) => permissionRowHtml(
      `practitioner:${practitioner.id}`,
      practitioner.name,
      permissionSettings.practitioners?.[practitioner.id] || ["agenda", "pacientes", "rendimiento"]
    ))
  ].join("");
...
function setupPatientDialog() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    if (!patientName) {
      const nameField = form.elements.name || form.elements.firstName;
      nameField.setCustomValidity("Escribe el nombre completo o rellena nombre y apellidos.");
      form.reportValidity();
