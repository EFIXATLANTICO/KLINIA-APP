...
const permissionSections = ["agenda", "pacientes", "equipo", "automatizaciones", "facturacion", "rendimiento", "estadisticas", "configuracion"];
const permissionLabels = {
  agenda: "Agenda",
  pacientes: "Pacientes",
  equipo: "Equipo y salas",
  automatizaciones: "Recordatorios",
  facturacion: "Facturacion",
  rendimiento: "Rendimiento",
  estadisticas: "Estadisticas",
  configuracion: "Disponibilidad / configuracion"
};

const defaultPermissionSettings = {
  staff: ["agenda", "pacientes", "equipo", "automatizaciones", "facturacion"],
  practitioners: {}
};
...
function normalizeClinicAccounts(accounts) {
  ...
  items.forEach((account) => {
    ...
    if (key) {
      byKey.set(key, { password: "demo", staffPassword: "demo", ...account, key });
    }
  });
...
function normalizePermissionSettings(savedPermissions) {
  ...
  return {
    staff: Array.isArray(saved.staff) ? saved.staff.filter((section) => permissionSections.includes(section)) : defaultPermissionSettings.staff,
    practitioners: saved.practitioners && typeof saved.practitioners === "object" ? saved.practitioners : {}
  };
}
...
function loadActiveClinicData(clinicKey = demoClinicKey) {
  ...
  patientPacks = loadClinicState("patient-packs", []);
...
function canManageOperations() {
  return isOwner() || permissionsForCurrentSession().includes("pacientes");
}

function canManageClinic() {
  return isOwner() || permissionsForCurrentSession().includes("configuracion");
}

function canViewDirectionReports() {
  return isOwner() || permissionsForCurrentSession().includes("rendimiento");
}
...
function visibleAppointments() {
  const ranged = appointments.filter((appointment) => isAppointmentInCurrentRange(appointment));
  if (!isPractitionerSession()) {
    return ranged;
  }
...
function groupPassesAgendaFilters(group) {
  ...
  return isBlockingAppointmentStatus(appointment.status) && visibleByPractitioner && visibleByRoom;
}
...
function practitionerIsFreeAt(practitioner, dateValue, hour) {
  ...
  return !isOutsidePractitionerHours(practitioner, hour, end)
    && !availabilityBlockFor(practitioner.id, dateValue, hour, end)
...
function renderWorkerColorLegend() {
  ...
  $$("[data-worker-jump]").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      $(".nav-item[data-section='agenda']").classList.add("active");
      $("#agenda").classList.add("active");
      $("#section-title").textContent = sectionTitles.agenda;
      renderFilters();
      renderSession();
      renderSchedule();
      renderNextList();
...
function renderSchedule() {
  ...
  hours.forEach((hour) => {
    ...
    visiblePractitioners.forEach((practitioner) => {
      ...
      } else if (!cell.classList.contains("outside-hours")) {
...
function renderPatients() {
  $("#patients-table").innerHTML = patients
    .map((patient) => `
      <tr>
        ...
        <td>${patient.last}</td>
        <td><span class="status-pill">${patient.status}</span></td>
        <td>
  ...
  $$(".patient-link").forEach((button) => {
    button.addEventListener("click", () => {
      ...
      renderPatientDetail();
      $("#patient-detail")?.scrollIntoView({ behavior: "smooth", block: "start" });
    });
  });
...
function renderPatientDetail() {
  ...
  const templateSelect = $("#patient-consent-template");
  if (templateSelect) {
    templateSelect.innerHTML = "";
    consentTemplates.forEach((template) => templateSelect.append(new Option(template.name, template.id)));
    templateSelect.disabled = !consentTemplates.length;
  }
  ...
  $("#patient-consents").innerHTML = consents.length
    ? consents.map((item) => `<article class="compact-item"><strong>${item.templateName}</strong><span>${item.createdAt} - ${item.signed ? "Firmado" : "Pendiente de firma"}</span></article>`).join("")
    : `<article class="compact-item"><span>Sin consentimientos asignados.</span></article>`;

  const packSelect = $("#patient-pack-template");
  if (packSelect) {
    packSelect.innerHTML = "";
    sessionPacks.forEach((pack) => packSelect.append(new Option(`${pack.name} - ${pack.sessions} sesiones - ${pack.price} EUR`, pack.id)));
  ...
  $("#patient-packs").innerHTML = packs.length
    ? packs.map((item) => `<article class="compact-item"><strong>${item.name}</strong><span>${item.used || 0}/${item.sessions} usadas - ${item.price} EUR ${item.invoice ? "- Facturable" : ""}</span></article>`).join("")
    : `<article class="compact-item"><span>Sin bonos asignados.</span></article>`;

  const notes = clinicalNotes.filter((note) => note.patientId === patient.id).sort((a, b) => b.id - a.id);
  $("#clinical-notes").innerHTML = notes.length
    ? notes
        .map((note) => `
          <article class="compact-item">
            <strong>${note.date} - ${note.author}${note.diagnosis ? ` - ${note.diagnosis}` : ""}</strong>
            <p>${note.content}</p>
            ${note.attachmentName ? `<span>Archivo: ${note.attachmentName}</span>` : ""}
          </article>
        `)
        .join("")
      ...
      return `
        <article class="compact-item availability-setting-card">
          <div>
            ...
            <span>${block.date} - ${availabilityBlockLabel(block)}</span>
          </div>
          <details class="item-menu">
    ...
    .map((service) => `
      <article class="compact-item service-setting-card action-card">
        <div>
          ...
          ${service.type === "group" ? `
            <span>Max. ${service.capacity || 1} integrantes - Mensual ${service.monthlyPrice || 0} EUR - Sesion suelta ${service.dropInPrice || service.price || 0} EUR - Comision ${service.commissionPerPatient || 0}%</span>
          ` : `
  ...
  $("#services-table").innerHTML = services
    .map((service) => `
      <tr>
        ...
        <td>
          ${service.type === "group"
            ? `Sesion grupal - ${service.capacity || 1} plazas - ${service.dropInPrice || service.price || 0} EUR/sesion - ${service.monthlyPrice || 0} EUR/mes - ${service.commissionPerPatient || 0}% prof.`
            : `${service.price || 0} EUR`}
        </td>
...
function renderStats() {
  ...
  const serviceList = $("#service-stats-list");
  if (!chart || !serviceList) return;

  const start = weekStartIso(selectedDate);
  const days = Array.from({ length: 7 }, (_, index) => addDaysIso(start, index));
  const dayLabels = ["L", "M", "X", "J", "V", "S", "D"];
  const counts = days.map((day) => appointments.filter((appointment) => (appointment.date || selectedDate) === day).length);
  const maxCount = Math.max(...counts, 0);
  chart.innerHTML = counts
    .map((count, index) => {
      const height = maxCount ? Math.max(8, Math.round((count / maxCount) * 100)) : 0;
      return `
        <div class="bar ${count ? "" : "bar-empty"}">
  ...
  const total = Array.from(serviceCounts.values()).reduce((sum, value) => sum + value, 0);
  serviceList.innerHTML = total
    ? Array.from(serviceCounts.entries())
        .sort((a, b) => b[1] - a[1])
        .slice(0, 6)
        .map(([name, count]) => `<p><span>${name}</span><strong>${Math.round((count / total) * 100)}%</strong></p>`)
        .join("")
    : `<p><span>Sin datos de esta clinica</span><strong>0%</strong></p>`;
}
...
function renderBilling() {
  ...
  const visible = [...appointmentRows, ...groupRows];
...
function practitionerReport(practitioner) {
  ...
  return {
    ...
    payout,
    occupancy,
    targetProgress: practitioner.target ? Math.min(100, Math.round((revenue / practitioner.target) * 100)) : 0
  };
}
...
function isFinalReminderStatus(status) {
  return ["sent", "confirmed", "failed"].includes(status);
}
...
function saveReminderAction(reminder, status) {
  const finalStatus = isFinalReminderStatus(status);
      ...
      ? consentTemplates.map((item) => `
        <article class="compact-item action-card">
          <div>
            ...
            <span>${item.body || "Plantilla de consentimiento del centro"}</span>
          </div>
          <div class="inline-actions"><button class="secondary-button" type="button" data-edit-consent="${item.id}">Editar</button><button class="danger-button" type="button" data-delete-consent="${item.id}">Eliminar</button></div>
        </article>
      `).join("")
    ...
    packList.innerHTML = sessionPacks.length
      ? sessionPacks.map((item) => `
        <article class="compact-item action-card">
          <div>
            <strong>${item.name}</strong>
            <span>${item.sessions} sesiones - ${item.price} EUR ${item.invoice ? "- Facturable" : ""}</span>
          </div>
          <div class="inline-actions"><button class="secondary-button" type="button" data-edit-pack="${item.id}">Editar</button><button class="danger-button" type="button" data-delete-pack="${item.id}">Eliminar</button></div>
        </article>
      `).join("")
...
function upsertConsentTemplate(existing = null) {
  const name = prompt("Nombre del consentimiento", existing?.name || "Consentimiento informado");
  if (!name) return;
  const body = prompt("Texto/base de la plantilla", existing?.body || "Autorizo el tratamiento fisioterapeutico y el uso de mis datos segun la politica del centro.");
  const next = { id: existing?.id || `consent-${Date.now()}`, name: name.trim(), body: (body || "").trim() || "Plantilla pendiente de completar" };
  consentTemplates = existing
    ? consentTemplates.map((item) => item.id === existing.id ? next : item)
    : [...consentTemplates, next];
...
function deleteConsentTemplate(id) {
  ...
  if (used && !confirm("Este consentimiento esta asignado a pacientes. Si lo eliminas, las asignaciones historicas se mantienen con su nombre. Continuar?")) return;
  if (!used && !confirm(`Eliminar ${template.name}?`)) return;
...
function upsertSessionPack(existing = null) {
  const name = prompt("Nombre del bono", existing?.name || "Bono 10 sesiones");
  if (!name) return;
  const sessions = Math.max(1, Number(prompt("Numero de sesiones", existing?.sessions || "10") || existing?.sessions || 10));
  const price = Math.max(0, Number(prompt("Precio del bono EUR", existing?.price || "400") || existing?.price || 0));
  const invoice = confirm("Marcar este bono como facturable?");
  const next = { id: existing?.id || `pack-${Date.now()}`, name: name.trim(), sessions, price, invoice };
  sessionPacks = existing
    ? sessionPacks.map((item) => item.id === existing.id ? next : item)
    : [...sessionPacks, next];
...
function deleteSessionPack(id) {
  ...
  if (used && !confirm("Este bono esta asignado a pacientes. Si lo eliminas, los bonos ya asignados se mantienen. Continuar?")) return;
  if (!used && !confirm(`Eliminar ${pack.name}?`)) return;
...
function applySectionChrome() {
  const agendaActive = activeSection === "agenda";
  $(".metrics-grid")?.classList.toggle("hidden", !agendaActive);
  $("#worker-color-legend")?.classList.toggle("hidden", !agendaActive || isPractitionerSession());
  const label = $("#current-date-label");
  if (label) {
    label.textContent = agendaActive ? formatDateLabel() : (sectionTitles[activeSection] || "Klinia");
  }
}
...
function permissionRowHtml(id, title, selectedSections, kind = "staff") {
  const selected = new Set(selectedSections || []);
  return `
    <article class="permission-row permission-table-row" data-permission-row="${id}">
      <div class="permission-profile">
        <strong>${title}</strong>
        <span>${kind === "staff" ? "Utilidades / recepcion" : "Trabajador"}</span>
      </div>
      <div class="permission-options permission-table-options">
        ${permissionSections.map((section) => `
          <label class="permission-check permission-toggle" title="${permissionLabels[section]}">
            <input type="checkbox" value="${section}" ${selected.has(section) ? "checked" : ""} data-permission-target="${id}" />
            <span>${permissionLabels[section]}</span>
          </label>
        `).join("")}
      </div>
    </article>
...
function renderPermissions() {
  ...
  list.innerHTML = [
    permissionRowHtml("staff", "Utilidades / recepcion", permissionSettings.staff, "staff"),
    ...practitioners.map((practitioner) => permissionRowHtml(`practitioner:${practitioner.id}`, practitioner.name, permissionSettings.practitioners?.[practitioner.id] || ["agenda", "pacientes", "rendimiento"], "practitioner"))
  ].join("");
...
function applyRolePermissions() {
  ...
  const activeHidden = !allowedSections.includes($(".nav-item.active")?.dataset.section);
  if (activeHidden) {
    const fallback = allowedSections.includes(activeSection) ? activeSection : allowedSections[0] || "agenda";
...
function setupLogin() {
  ...
  $("#register-google").addEventListener("click", () => {
    ...
    const email = window.prompt("Email de Google para direccion", form.elements.email.value || "");
    if (!email) {
      return;
    }
  ...
  $("#register-form").addEventListener("submit", (event) => {
    ...
    const account = {
      ...
      staffPassword: "demo",
      paymentPlan: form.elements.paymentPlan?.value || "trial",
      billingStatus: form.elements.paymentPlan?.value === "trial" ? "trial" : "pending_stripe"
    };
    clinicAccounts = normalizeClinicAccounts([...clinicAccounts, account]);
    saveClinicAccounts();

    activeClinicKey = key;
    saveClinicState("clinic", { name: account.name, email: account.email, phone: account.phone });
...
function openAppointmentDialog(defaults = {}) {
  ...
  if (form.elements.repeatWeekly) {
    ...
    form.elements.repeatCount.value = 4;
...
function setupDialog() {
  ...
  form.elements.service.addEventListener("change", () => updateAppointmentGroupAttendeesVisibility(form));
  ...
  form.addEventListener("submit", (event) => {
    ...
    const blockedByAvailability = availabilityBlockFor(candidate.practitionerId, candidate.date, candidate.start, candidateEnd);
    if (blockedByAvailability) {
      $("#form-error").textContent = `${practitioner?.name || "El profesional"} esta marcado como ${availabilityBlockLabel(blockedByAvailability)}.`;
      $("#form-error").classList.add("visible");
      return;
    }

    const outsideHours = !isWithinAvailability(candidate);
    if (outsideHours && !window.confirm(`Esta creando una cita fuera de horario. ${practitioner?.name || "El profesional"} trabaja en este horario: ${practitionerAvailabilityLabel(practitioner)}. Quieres guardarla igualmente?`)) {
      $("#form-error").textContent = "Cita fuera de horario no guardada.";
      $("#form-error").classList.add("visible");
      return;
    }

    const repeatCount = form.elements.repeatWeekly?.checked ? Math.max(2, Math.min(52, Number(form.elements.repeatCount?.value || 2))) : 1;
    const candidates = Array.from({ length: repeatCount }, (_, index) => ({
      ...candidate,
      id: index === 0 ? candidate.id : `local-${Date.now()}-${index}`,
      date: addDaysIso(candidate.date, index * 7)
    }));

    const conflictCandidate = candidates.find((item) => findConflict(item));
    if (conflictCandidate) {
      const conflict = findConflict(conflictCandidate);
      const patient = byId(patients, conflict.patientId)?.name || "otra cita";
      $("#form-error").textContent = `Conflicto detectado el ${conflictCandidate.date} con la cita de ${patient} a las ${conflict.start}.`;
      $("#form-error").classList.add("visible");
      return;
    }

    const groupConflictCandidate = candidates.find((item) => findGroupConflict(item));
    if (groupConflictCandidate) {
      const groupConflict = findGroupConflict(groupConflictCandidate);
      $("#form-error").textContent = `Ese hueco esta bloqueado el ${groupConflictCandidate.date} por el grupo ${groupConflict.name} a las ${groupConflict.start}.`;
      $("#form-error").classList.add("visible");
      return;
    }

    const finish = (newAppointments) => {
    ...
    if (apiEnabled && isDemoClinic() && repeatCount === 1 && candidate.serviceId && byId(services, candidate.serviceId)) {
      apiRequest("/api/appointments", {
        method: "POST",
        body: JSON.stringify(uiAppointmentToApi(candidate))
      })
        .then((appointment) => finish(apiAppointmentToUi(appointment)))
        .catch(() => finish(candidate));
      return;
    }
...
function openAppointmentDetail(appointmentId) {
  ...
  const service = byId(services, appointment.serviceId);
  $("#appointment-detail-title").textContent = `${byId(patients, appointment.patientId)?.name || "Paciente"} - ${appointment.start}`;
...
function setupAppointmentDetail() {
  ...
  $("#appointment-detail-form").addEventListener("submit", (event) => {
    event.preventDefault();
    const form = event.currentTarget;
    const statusChangedToCancelled = form.elements.status.value === "cancelled";
    const cancelledBy = statusChangedToCancelled
    ...
    const localUpdate = {
      ...byId(appointments, selectedAppointmentId),
      status: form.elements.status.value,
      internalNotes: form.elements.internalNotes.value.trim(),
      cancelledBy,
      cancelledAt: statusChangedToCancelled ? (byId(appointments, selectedAppointmentId)?.cancelledAt || new Date().toISOString()) : ""
    };
    ...
    if (apiEnabled && isDemoClinic() && !String(selectedAppointmentId).startsWith("local-")) {
      apiRequest(`/api/appointments/${selectedAppointmentId}/status`, {
        ...
        .then((appointment) => finish({ ...apiAppointmentToUi(appointment), cancelledBy, cancelledAt: localUpdate.cancelledAt, invoiceGenerated: localUpdate.invoiceGenerated }))
...
function openPatientEditor(patientId) {
  ...
  setFormValue(form, "firstName", patient.firstName || "");
  setFormValue(form, "lastName", patient.lastName || "");
  setFormValue(form, "name", patient.name || "");
  setFormValue(form, "dni", patient.dni || "");
  setFormValue(form, "sex", patient.sex || "");
  setFormValue(form, "birthDate", patient.birthDate || "");
  setFormValue(form, "occupation", patient.occupation || "");
  setFormValue(form, "phone", patient.phone || "");
  setFormValue(form, "email", patient.email || "");
  setFormValue(form, "address", patient.address || "");
  setFormValue(form, "alert", patient.alert || "");
...
function setupPatientDialog() {
  ...
  form.addEventListener("submit", (event) => {
    ...
    const finish = (patient) => {
      patients = form.dataset.editingPatientId
    ...
    if (!form.dataset.editingPatientId && apiEnabled && isDemoClinic()) {
      apiRequest("/api/patients", {
        ...
        .then((patient) => finish(apiPatientToUi(patient)))
        .catch(() => finish(localPatient));
      return;
    }
...
function setupConfiguration() {
  ...
  $("#delete-clinic").addEventListener("click", () => {
    if (isDemoClinic()) {
      const confirmed = window.confirm("Esto limpiara la demo local y la dejara con datos de ejemplo. Quieres continuar?");
      if (!confirmed) {
        return;
      }
      ...
      const confirmed = window.confirm(`Vas a resetear pacientes, citas, trabajadores, salas y servicios de ${clinic.name}. La clinica seguira existiendo para poder entrar de nuevo. Quieres continuar?`);
      if (!confirmed) {
        return;
      }
...
function setupPatientConsentsAndPacks() {
  $("#add-patient-consent")?.addEventListener("click", () => {
    const patient = byId(patients, selectedPatientId);
    const template = byId(consentTemplates, $("#patient-consent-template")?.value);
    if (!patient || !template) {
      alert("Crea primero una plantilla de consentimiento en Configuracion.");
      return;
    }
    patientConsents = [...patientConsents, {
      id: `patient-consent-${Date.now()}`,
      patientId: patient.id,
      templateId: template.id,
      templateName: template.name,
      createdAt: new Date().toLocaleString("es-ES"),
      signed: false,
      fiscalData: { name: patient.name, dni: patient.dni || "", address: patient.address || "" }
    }];
    saveClinicState("patient-consents", patientConsents);
    renderPatientDetail();
  });

  $("#patient-signature-pad")?.addEventListener("click", () => {
    if (!selectedPatientId) return;
    const latest = patientConsents.find((item) => item.patientId === selectedPatientId && !item.signed);
    if (!latest) {
      alert("Asigna primero un consentimiento pendiente.");
      return;
    }
    patientConsents = patientConsents.map((item) => item.id === latest.id ? { ...item, signed: true, signedAt: new Date().toISOString() } : item);
    saveClinicState("patient-consents", patientConsents);
    renderPatientDetail();
  });
  ...
  $("#assign-patient-pack")?.addEventListener("click", () => {
    ...
    if (!patient || !pack) {
      alert("Crea primero un bono en Configuracion.");
      return;
    }
...
function setupPatientDetail() {
  ...
  $("#clinical-note-form").addEventListener("submit", (event) => {
    event.preventDefault();
