...
function deleteClinicStorage(clinicKey) {
  ["clinic", "patients", "appointments", "clinical-notes", "services", "practitioners", "rooms", "groups", "availability-blocks", "group-dropins", "group-completions", "permissions", "reminder-actions", "reminder-settings"].forEach((key) => {
...
function renderPatientDetail() {
    ...
    ? patientAppointments
        .map((appointment) => `
          <article class="compact-item">
            <strong>${appointment.start} - ${byId(services, appointment.serviceId)?.name || "Servicio no encontrado"}</strong>
  ...
  $("#clinical-notes").innerHTML = notes.length
    ? notes
        .map((note) => `
          <article class="compact-item">
            <strong>${note.date} - ${note.author}</strong>
            <p>${note.content}</p>
          </article>
        `)
        .join("")
