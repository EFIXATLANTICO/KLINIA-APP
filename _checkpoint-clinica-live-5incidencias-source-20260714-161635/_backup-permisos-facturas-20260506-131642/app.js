...
function apiPatientToUi(patient) {
  return {
    ...
    alert: patient.internal_alert || "Sin alertas relevantes"
  };
}

function uiPatientToApi(form) {
  const parts = form.elements.name.value.trim().split(/\s+/);
  ...
  return {
    ...
    internal_alert: form.elements.alert.value.trim() || null
  };
}
...
function renderCommercialSettings() {
  ...
  if (consentList) {
    consentList.innerHTML = consentTemplates.length
      ? consentTemplates.map((item) => `<article class="compact-item"><strong>${item.name}</strong><span>Plantilla de consentimiento del centro</span></article>`).join("")
      : `<article class="compact-item"><span>Sin consentimientos creados.</span></article>`;
  }
  if (packList) {
    packList.innerHTML = sessionPacks.length
      ? sessionPacks.map((item) => `<article class="compact-item"><strong>${item.name}</strong><span>${item.sessions} sesiones - ${item.price} EUR ${item.invoice ? "- Facturable" : ""}</span></article>`).join("")
      : `<article class="compact-item"><span>Sin bonos creados.</span></article>`;
  }
}

function setupCommercialSettings() {
  $("#new-consent-template")?.addEventListener("click", () => {
    const name = prompt("Nombre del consentimiento", "Consentimiento informado");
    if (!name) return;
    consentTemplates = [...consentTemplates, { id: `consent-${Date.now()}`, name: name.trim(), body: "Plantilla pendiente de completar" }];
    saveClinicState("consent-templates", consentTemplates);
    renderCommercialSettings();
  });
  $("#new-session-pack")?.addEventListener("click", () => {
    const name = prompt("Nombre del bono", "Bono 10 sesiones");
    if (!name) return;
    const sessions = Math.max(1, Number(prompt("Numero de sesiones", "10") || 10));
    const price = Math.max(0, Number(prompt("Precio del bono EUR", "400") || 0));
    sessionPacks = [...sessionPacks, { id: `pack-${Date.now()}`, name: name.trim(), sessions, price, invoice: true }];
...
function openAppointmentDetail(appointmentId) {
  ...
  $("#appointment-detail-data").innerHTML = `
    ...
    ${appointment.invoiceGenerated ? `<dt>Factura</dt><dd>Generada</dd>` : ""}
  `;
...
function setupAppointmentDetail() {
  ...
  $("#appointment-invoice-button")?.addEventListener("click", () => {
    ...
    appointments = appointments.map((item) => item.id === selectedAppointmentId ? { ...item, invoiceGenerated: true, invoiceGeneratedAt: new Date().toISOString() } : item);
    saveClinicState("appointments", appointments);
    alert("Factura marcada como generada. La integracion fiscal real se conectara en la fase de facturacion.");
