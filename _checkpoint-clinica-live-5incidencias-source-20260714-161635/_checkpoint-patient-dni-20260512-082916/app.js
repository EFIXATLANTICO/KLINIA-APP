...
function renderTwoMonthPlanner(schedule, range) {
  ...
  monthStarts.forEach((monthStart) => {
    ...
    month.innerHTML = `
      <h3>${new Intl.DateTimeFormat("es-ES", { month: "long", year: "numeric" }).format(dateOnly(monthStart))}</h3>
...
function formatDateLabel() {
  ...
  const range = calendarRange();
  return `Planificador: ${new Intl.DateTimeFormat("es-ES", { month: "long" }).format(dateOnly(range.start))} y ${new Intl.DateTimeFormat("es-ES", { month: "long", year: "numeric" }).format(dateOnly(addMonthsIso(range.start, 1)))}`;
}
