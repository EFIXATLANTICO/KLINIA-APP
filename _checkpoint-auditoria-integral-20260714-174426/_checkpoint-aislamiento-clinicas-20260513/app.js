...
function ensureClinicAccount(account) {
  clinicAccounts = normalizeClinicAccounts([
    ...
    { password: "", staffPassword: "", staffEmail: "", ...account }
  ]);
  saveClinicAccounts();
}
...
function renderLoginProfiles() {
  ...
  const selectedKey = pendingClinicKey || clinicAccountByLogin(clinicInput.value)?.key || "";
...
function applySavedLoginCredentials() {
  ...
  if (clinicAccountByLogin(saved.center)?.key === demoClinicKey) {
...
function resetRegisterFlow(planId = "trial") {
  ...
  form.elements.ownerName.value = "Direccion";
  form.elements.ownerRole.value = "Direccion";
  form.elements.password.value = "demo";
  form.elements.confirmPassword.value = "demo";
...
function setupPublicAccessNavigation() {
  ...
  $("#forgot-password")?.addEventListener("click", () => {
    showNotice(
      "Restablecer acceso",
      "En esta version local, el restablecimiento se gestiona desde Direccion. En produccion se enviara un enlace seguro por email.",
      { variant: "info" }
    );
  });
