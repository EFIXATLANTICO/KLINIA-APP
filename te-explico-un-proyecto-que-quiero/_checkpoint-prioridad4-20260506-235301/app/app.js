...
const saasPlans = [
  { id: "trial", name: "Demo", price: 0, interval: "14 dias", summary: "Demo comercial - 14 dias" },
  { id: "starter", name: "Starter", price: 59, interval: "mes", summary: "Starter mensual - 59 EUR/mes" },
  { id: "pro", name: "Pro", price: 119, interval: "mes", summary: "Pro mensual - 119 EUR/mes" },
  { id: "business", name: "Business", price: 199, interval: "mes", summary: "Business mensual - 199 EUR/mes" }
];
...
const permissionSections = ["agenda", "pacientes", "equipo", "automatizaciones", "facturacion", "rendimiento", "estadisticas", "configuracion", "disponibilidad"];
...
const defaultClinicAccount = {
  ...
  billingStatus: "trial",
  trialEndsAt: addDaysIso(todayIso(), 14),
  billingProfile: {
...
function normalizeClinicAccounts(accounts) {
  ...
  items.forEach((account) => {
    ...
    if (key) {
      const plan = account.paymentPlan || "trial";
      byKey.set(key, {
        password: "demo",
        staffPassword: "demo",
        paymentPlan: plan,
        billingStatus: plan === "trial" ? "trial" : "pending_stripe",
        subscriptionStatus: plan === "trial" ? "trialing" : "incomplete",
        trialEndsAt: addDaysIso(todayIso(), 14),
        checkoutUrl: "",
        stripeCustomerId: "",
...
function openDataUrlDocument(dataUrl, filename = "documento") {
  ...
  if (!win) return;
  win.document.write(`<!doctype html><html lang="es"><head><meta charset="utf-8"><title>${filename}</title><style>body{margin:0;font-family:Arial,sans-serif;background:#f7f7f4;color:#202621}iframe,img{display:block;width:100%;height:100vh;border:0;object-fit:contain}</style></head><body>${dataUrl.startsWith("data:image/") ? `<img src="${dataUrl}" alt="${filename}">` : `<iframe src="${dataUrl}" title="${filename}"></iframe>`}</body></html>`);
  win.document.close();
}
...
function renderPatientDetail() {
  ...
  $("#patient-consents").innerHTML = consents.length
    ? consents.map((item) => `
      <article class="compact-item consent-card">
        <strong>${item.templateName}</strong>
        <span>${item.createdAt} - ${item.signed ? `Firmado ${item.signedAt ? new Date(item.signedAt).toLocaleString("es-ES") : ""}` : "Pendiente de firma"}</span>
        <p>${item.body || "Consentimiento asignado al paciente."}</p>
  ...
  $("#patient-packs").innerHTML = packs.length
    ? packs.map((item) => `<article class="compact-item"><strong>${item.name}</strong><span>${patientPackRemaining(item)} disponibles de ${item.sessions} - ${item.price} EUR - ${packServiceLabel(item)} ${item.invoice ? "- Facturable" : ""}</span></article>`).join("")
  ...
  if (invoicesList) {
    ...
    `), ...invoicePacks.map((pack) => `
      <article class="compact-item">
        <strong>Bono pendiente de factura - ${pack.name}</strong>
    ...
    ? notes
        .map((note) => `
          <article class="compact-item">
            ...
            <p>${note.content}</p>
            ${note.attachmentName ? `<button class="secondary-button compact-inline-button" type="button" data-open-note-file="${note.id}">Abrir ${note.attachmentName}</button>` : ""}
          </article>
        `)
        .join("")
...
function renderSaasSettings() {
  ...
  if (options) {
    options.innerHTML = saasPlans.filter((item) => item.id !== "business").map((item) => `
...
function setupSaasSettings() {
  ...
  $("#start-subscription")?.addEventListener("click", () => {
    const account = currentClinicAccount();
    const selectedPlan = account.paymentPlan === "trial" ? "starter" : account.paymentPlan;
    clinicAccounts = normalizeClinicAccounts(clinicAccounts.map((item) => (
      item.key === activeClinicKey
        ? {
            ...item,
...
function upsertSessionPack(existing = null) {
  ...
  const serviceOptions = services.map((service) => `${service.id}: ${service.name}`).join("\n");
  const serviceAnswer = prompt(`Servicio asociado opcional. Escribe id o nombre, o deja vacio para todos:\n${serviceOptions}`, existing?.serviceId || "");
  const selectedService = services.find((service) => service.id === serviceAnswer || service.name.toLowerCase() === String(serviceAnswer || "").trim().toLowerCase());
  const invoice = confirm("Marcar este bono como facturable?");
  const next = { id: existing?.id || `pack-${Date.now()}`, name: name.trim(), sessions, price, serviceId: selectedService?.id || "", invoice };
...
function setupLogin() {
  ...
  $("#register-form").addEventListener("submit", (event) => {
    ...
    const account = {
      ...
      trialEndsAt: paymentPlan === "trial" ? addDaysIso(todayIso(), 14) : "",
...
function openAppointmentDetail(appointmentId) {
  ...
  if (packField && packSelect) {
    ...
    if (appointment.patientPackId && !availablePacks.some((pack) => pack.id === appointment.patientPackId)) {
      const usedPack = byId(patientPacks, appointment.patientPackId);
      if (usedPack) packSelect.append(new Option(`${usedPack.name} - aplicado`, usedPack.id));
    }
    packSelect.value = appointment.patientPackId || "";
    packField.classList.toggle("hidden", packSelect.options.length <= 1 && !appointment.patientPackId);
  }
  ...
  $("#appointment-detail-form").addEventListener("submit", (event) => {
    ...
    if (apiEnabled && isDemoClinic() && !String(selectedAppointmentId).startsWith("local-")) {
      apiRequest(`/api/appointments/${selectedAppointmentId}/status`, {
        ...
        .then((appointment) => finish({ ...apiAppointmentToUi(appointment), cancelledBy, cancelledAt: localUpdate.cancelledAt, invoiceGenerated: localUpdate.invoiceGenerated, patientPackId: localUpdate.patientPackId, patientPackUsedAt: localUpdate.patientPackUsedAt }))
...
function consentBodyForPatient(template, patient) {
...
function setupSignatureCanvas() {
  const canvas = $("#patient-signature-canvas");
  ...
  $("#clear-patient-signature")?.addEventListener("click", () => {
    context.clearRect(0, 0, canvas.width, canvas.height);
  });
  $("#save-patient-signature")?.addEventListener("click", () => {
    ...
    patientConsents = patientConsents.map((item) => item.id === latest.id
      ? { ...item, signed: true, signedAt: new Date().toISOString(), signatureData: canvas.toDataURL("image/png") }
      : item
    );
...
function setupPatientConsentsAndPacks() {
  ...
  $("#add-patient-consent")?.addEventListener("click", () => {
    ...
    patientConsents = [...patientConsents, {
      ...
      body: consentBodyForPatient(template, patient),
      createdAt: new Date().toLocaleString("es-ES"),
      signed: false,
